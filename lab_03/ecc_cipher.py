...
class MyApp(QMainWindow):
    ...
    def call_api_sign(self):
        ...
        payload = {
            "message": self.ui.textEdit.toPlainText(),  # Sửa từ txt_info
        }
        try:
            response = requests.post(url, json=payload)
            if response.status_code == 200:
                data = response.json()
                self.ui.textEdit_2.setText(data["signature"])  # Sửa từ txt_sign
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Information)
    ...
    def call_api_verify(self):
        ...
        payload = {
            "message": self.ui.textEdit.toPlainText(),      # Sửa từ txt_info
            "signature": self.ui.textEdit_2.toPlainText()   # Sửa từ txt_sign
        }
        try:
            response = requests.post(url, json=payload)
            if response.status_code == 200:
